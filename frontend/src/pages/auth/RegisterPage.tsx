    ...
    <PageShell>
      <div className="flex w-full max-w-md flex-col gap-6 rounded-3xl bg-white p-8 shadow-2xl dark:bg-slate-900 mx-auto mt-12 md:mt-24">
        <div className="flex flex-col items-center text-center">
          ...
          <h1 className="text-2xl font-black text-ink dark:text-white">Tạo tài khoản</h1>
          <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">
            Hành trình khám phá bản ngã bắt đầu.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <div>
            ...
            <input
              ...
              className="w-full rounded-xl border-2 border-slate-200 bg-transparent px-4 py-3 text-ink outline-none transition-all focus:border-indigo-500 dark:border-slate-800 dark:text-white dark:focus:border-indigo-500"
          ...
          <button
            type="submit"
            disabled={loading}
            className="mt-2 flex w-full items-center justify-center gap-2 rounded-xl bg-indigo-500 px-6 py-4 font-bold text-white shadow-lg shadow-indigo-500/30 transition-all hover:-translate-y-1 hover:shadow-indigo-500/50 disabled:opacity-50 disabled:hover:translate-y-0"
          >
